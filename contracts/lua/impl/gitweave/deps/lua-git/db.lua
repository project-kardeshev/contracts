...
[gc]
	...
	function db.resolve(ref)
		...
		return db.getRef(ref) or db.getRef("refs/heads/" .. ref) or db.getRef("refs/tags/" .. ref)
	end
	...
	function db.nodes(prefix)
		return commonIter(storage.nodes(prefix), makePackedIter(prefix, true))
	end

	function db.leaves(prefix)
		return commonIter(storage.leaves(prefix), makePackedIter(prefix, false))
	end
