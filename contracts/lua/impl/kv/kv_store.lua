...
function kv_store.init()
	...
	-- TODO: for AOS implementations, this needs to have an `initialized` state
	-- and only set the admin if no initialized to avoid overwriting any intentional renouncing of admin control
	...
	--- TODO: guard read-only addresses with burn address ("0000000000000000000000000000000000000000000")
	--- This will make it so only client side apps can read the state.
	--- To enable other processes to read the contract, should add paginated read handlers
	--- This is to ensure that no process can blow up the memory accidentally or intentionally once the state stored becomes large.
